<?php
	...
	for($i = 0; $i < count($ary); $i++){
		...
		$image = '';
			...
			if($live[$ary[$i]]){
				if($live_image[$ary[$i]]){
					$image = '<div class="live_image" style="height:120px; text-align:center; overflow: hidden;"><img src="http://plv-artsalon.com/schedule_data/upload_file/'.$live_image[$ary[$i]].'" style="height:100%; width: auto;"></div>';
				}
...
for($i = 0; $i < 5; $i++){
	...
	$month_after .= '<div>'.$year_class.'<a href="?year='.$year_.'&month='.$t.'">'.$t.'</a></div>';
	if($t == 12){
		$t = 0;
	}
	$t++;
...
for($i = 0; $i < 5; $i++){
	...
	$month_before .= '<div>'.$year_class.'<a href="?year='.$year_.'&month='.$t.'">'.$t.'</a></div>';
	$t--;
}
...
<html>
	<head>
		...
		<link rel="stylesheet" media="screen and (max-width: 640px)" href="index_sp.css">
	...
	<body>
		<div id="calender">
			...
			<div id="month_select">
				<div id="prev"><a href="<?= $prev_btn; ?>" class="month_btn">≪ Prev</a></div>
				<?= $month_sel; ?>
				<div id="this"><a href="./" class="month_btn">This Month</a></div>
				<div id="next"><a href="<?= $next_btn; ?>" class="month_btn">Next ≫</a></div>
			</div>
